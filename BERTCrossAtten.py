...
from bert_model import BertLayer, BertAttention
...
class GatedDSlayerWithFeatures(nn.Module):
    ...
    def forward(self, fea1, fea2, uncer1, uncer2):
        ...
        #
        # else:
        #     # tensor1 = torch.tensor(com_belief['A']).to(device)
...
class MDFTransformer(nn.Module):
    def __init__(self):
        ...
        self.cross_modal_image_layers = nn.ModuleList(
            [BertLayer(bert_config) for _ in range(config.num_top_layer)])
        ...
        self.Gatelayer = GatedDSlayerWithFeatures()
